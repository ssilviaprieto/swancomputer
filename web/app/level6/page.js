...
import wallTextureUrl from './wall.jpg'
...
class Room {
  ...
  createRoom() {
    ...
    const imgUrl = (typeof wallTextureUrl === 'string') ? wallTextureUrl : (wallTextureUrl?.src || '')
    const texture = texLoader.load(imgUrl)
    ...
    const backWall = new THREE.Mesh(backWallGeometry, mat)
    backWall.position.z = -this.ROOM_SIZE / 2
    backWall.position.y = wallHeight / 2
    backWall.receiveShadow = true
    this.scene.add(backWall)
    const frontWall = new THREE.Mesh(backWallGeometry, mat)
    ...
    const leftWall = new THREE.Mesh(sideWallGeometry, mat)
    leftWall.position.x = -this.ROOM_SIZE / 2
    leftWall.position.y = wallHeight / 2
    leftWall.rotation.y = Math.PI / 2
    leftWall.receiveShadow = true
    this.scene.add(leftWall)
    const rightWall = new THREE.Mesh(sideWallGeometry, mat)
  ...
  createLights() {
    ...
    const hemi = new THREE.HemisphereLight(0xaaaaaa, 0x333333, 0.5)
    this.scene.add(hemi)
  ...
  loadCat() {
    ...
    loader.load(
      ...
      progress => { try { console.log('Loading model...', (progress.loaded / progress.total * 100) + '%') } catch {} },
      error => { try { console.error('Error loading peacock model:', error) } catch {} }
    )
  }
  setupEventListeners() {
    ...
    document.addEventListener('mousemove', e => {
      if (this.camera && this.controlsEnabled) {
        ...
        if (newH >= -this.ROTATION_LIMITS.horizontal && newH <= this.ROTATION_LIMITS.horizontal) {
          this.cameraRotation.horizontal = newH
        ...
        if (newV >= -this.ROTATION_LIMITS.vertical && newV <= this.ROTATION_LIMITS.vertical) {
          this.cameraRotation.vertical = newV
          this.camera.rotation.x = this.cameraRotation.vertical
        }
      }
    })
    const canvas = this.renderer.domElement
    if (canvas) canvas.addEventListener('click', () => { this.controlsEnabled = true })
  ...
  updateFloatingTexts() {
    ...
    for (let i = this.textSprites.length - 1; i >= 0; i--) { const sprite = this.textSprites[i]; const age = t - sprite.userData.birthTime; if (age >= sprite.userData.lifetime) { this.scene.remove(sprite); this.textSprites.splice(i,1); continue } if (age > sprite.userData.fadeOutStart) { const fp = (age - sprite.userData.fadeOutStart) / (sprite.userData.lifetime - sprite.userData.fadeOutStart); sprite.material.opacity = 1 - fp } sprite.quaternion.copy(this.camera.quaternion); sprite.position.y += Math.sin(t * 0.006) * 0.002 }
  }
  createFloatingText() {
  ...
  animate() { requestAnimationFrame(this.animate.bind(this)); const now = performance.now(); const delta = Math.min(0.05, (now - this.prevTime) / 1000); this.prevTime = now; this.updateMovement(delta); this.updateFloatingTexts(); if (this.peacock && this.spotlight) { this.spotlight.target = this.peacock; this.peacock.rotation.y += 0.05; this.peacock.rotation.x = Math.sin(Date.now() * 0.005) * 0.1; this.peacock.rotation.z = Math.cos(Date.now() * 0.005) * 0.1 } this.renderer.render(this.scene, this.camera) }
}
...
export default function Level6Page() {
  ...
  useEffect(() => { const r = new Room(); return () => { /* optional cleanup */ } }, [])

  useEffect(() => {
    const onKey = (e) => {
      if (e.code === 'Escape') { armedRef.current = true; bufferRef.current = ''; return }
      if (!armedRef.current) return
      if (!e.key || e.key.length !== 1) return
      const ch = e.key.toLowerCase()
      if (!/[a-z]/.test(ch)) return
      bufferRef.current = (bufferRef.current + ch).slice(-4)
      if (bufferRef.current === 'swan') { setOpen(true); armedRef.current = false; bufferRef.current = '' }
    }
  ...
  return (
    <main style={{ width: '100vw', height: '100vh', background: '#000', overflow: 'hidden' }}>
      <canvas id="room" style={{ width: '100%', height: '100%', display: 'block' }} />
      <Terminal open={open} onClose={() => setOpen(false)} mode="level6" />
    </main>
  )
}
