...
class Room {
    constructor() {
        ...
        this.MOUSE_SENSITIVITY = {
            horizontal: 0.002,  // Original horizontal sensitivity
            vertical: 0.0015    // Reduced vertical sensitivity
        };
    ...
    updateMovement() {
        ...
        if (direction.length() > 0) {
            ...
            // Use horizontal rotation for movement
            const rotatedMoveX = moveX * Math.cos(this.cameraRotation.horizontal) + 
                               moveZ * Math.sin(this.cameraRotation.horizontal);
            const rotatedMoveZ = moveZ * Math.cos(this.cameraRotation.horizontal) - 
                               moveX * Math.sin(this.cameraRotation.horizontal);
            
            const newPosition = this.camera.position.clone();
            newPosition.x += rotatedMoveX;
            newPosition.z += rotatedMoveZ;
            
            // Only update if within room bounds
            if (Math.abs(newPosition.x) < this.ROOM_SIZE/2 - 0.5 && 
                Math.abs(newPosition.z) < this.ROOM_SIZE/2 - 0.5) {
